...
def get_coords(wp_info):
    ...
    if get_name(wp_info) == 'Malaysia':
        m = re.match('(?i).*{{coord\s*\|([^}]*)}}','{{coord|3|8|N|101|41|E|display|=|inline}}' )
        str_coords = m.group(1)
        if str_coords[0:1] in '0123456789':
            return cv_coords(str_coords)
...
def save_country(conn,country,info,conti,compteur):
...
#    
    ...
    c.execute(sql,(country, name, capital, coords['lat'],coords['lon'],idc)
    conn.commit()
...
for i in range(len(paysAmSud)):
    compteur +=1
    L = get_zip_info(paysAmSud[i],'south_america')
    M = get_name(L)
    save_country(conn,M,L,'South America')    
    
for i in range(len(paysOceanie)):
    L = get_zip_info(paysAmSud[i],'oceania')
    M = get_name(L)
    save_country(conn,M,L,'Oceania')    
