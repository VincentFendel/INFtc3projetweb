...
class RequestHandler(http.server.SimpleHTTPRequestHandler):
  ...
  def do_GET(self):
    ...
    # ou par /service/country/...
      ...
      sortie = False
      for d in data:
        if d['id'] == int(self.path_info[1]) and sortie == False:
            print(2)
            self.send_json(d)
            sortie = True
  ...
  def db_get_countries(self):
    c = conn.cursor()
    sql = 'SELECT wp, name, capital, latitude, longitude, id, flag, continent FROM countries' 
